...
public:
    /* Enumeration */
    /** \brief Defines several possible options for camera 
     * movement. Used as abstraction to stay away from 
    ...
    //! Look from posotive z to negative z.
    ...
    bool debug1 = false;
    /* Functions  */
    /** \brief A constructor with vectors to define viewer 
     * position and focus.
    ...
    /** \brief A constructor with values to define viewer 
     * postion and Euler angles for direction.
     * width : the width of the viewport.
     * height : the height of the viewport.
     ...
     */
    Camera(int width, int height, float posX, float posY, 
           float posZ, float upX, float upY, 
           float upZ, float yaw, float pitch);
    /** \brief Destructor to complete the destruction of the class.
     */
    ~Camera();
    ...
    /** \brief Processes input received from any keyboard-like 
     * input system.  Accepts input parameter in the form of 
     * camera defined ENUM such as Camera::Camera_Movement::RIGHT 
     * or Camera::Camera_Movement::UP  (to abstract it from 
     * windowing systems). This is adjustable using the 
     * SPEED variable.
     */
    ...
    /** \brief Processes input received from a mouse 
     * scroll-wheel event.  Only requires input on the 
     * wheel-axis.
     */
