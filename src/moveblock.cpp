/**************************************************************
 * MoveBlock:  A class that uses OpenGL Transformation Feedback
 * to translate and scale an object and draw it to the screen.
...
void MoveBlock::initEmitParticles ()
{

   {
      ...
      // Link program must occur after calling glMoveBlockVaryings
      emitShader->linkShader();

   }
}
void MoveBlock::debug()
{
...
int MoveBlock::init()
{
    ...
    image->setImage("/usr/share/openglresources/images/container.png" );
...
void MoveBlock::setupVertexAttributes(GLuint vboID)
{
...
//
void MoveBlock::update(vec3 movement, float deltaTime)
{
    systime += deltaTime;
    if(debug1)
    {
...
void MoveBlock::draw(int width, int height, mat4 model, mat4 view, mat4 projection)
{
...
void MoveBlock::printMat4(mat4 val)
{
    for (int y = 0; y < 4; y++)
    {
        cout << "\n\t";
